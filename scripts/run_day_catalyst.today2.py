...
def save_trades_breakdown(out_dir: Path, rows, totals, gap_map, preflight_band, news_map, news_path):
    ...
    # keep columns aligned: 7 empties between TOTALS and 'trades'
    ...
    with csvp.open("w", newline="", encoding="utf-8") as f:
        ...
        # 7 empties between TOTALS and numbers to align with 12 columns
        w.writerow(["TOTALS"] + [""]*7 + [totals["trades"], totals["tp"], totals["sl"], totals["pnl_sum"]])
...
def main():
    ...
    # price_min/price_max available via scanner.price_min/price_max if needed later
    ...
    # 9) Preflight gap band + messages (from validated JSON)
    ...
    # 14) Per-symbol (with inclusion + news_score + in_band + type)
