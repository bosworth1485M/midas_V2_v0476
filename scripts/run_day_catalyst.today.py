...
# ───────────────────────────── Scenario gap band (tolerant + nested) ───────────────
def read_scenario_gap_band(scenario: str):
    """Tolerates min/max key variants; looks in root or nested blocks."""
    try:
        doc = json.loads(Path("config/scenarios.json").read_text(encoding="utf-8"))
        def pick(cfg, keys):
            for k in keys:
                if k in cfg: return cfg[k]
            return None
        containers = [doc]
        for k in ("scenarios", "config", "profiles"):
            if isinstance(doc.get(k, None), dict):
                containers.append(doc[k])
        cfg = {}
        for bag in containers:
            if not isinstance(bag, dict): continue
            cfg = bag.get(scenario) or bag.get(scenario.upper()) or bag.get(scenario.lower()) or {}
            if cfg: break
        if not isinstance(cfg, dict): return (None, None)
        minv = pick(cfg, ["min_gap_pct","gap_min","min_gap","min_gap_percent"])
        maxv = pick(cfg, ["max_gap_pct","gap_max","max_gap","max_gap_percent"])
        minv = float(minv) if minv is not None else None
        maxv = float(maxv) if maxv is not None else None
        return (minv, maxv)
    except Exception:
        return (None, None)
...
def per_symbol_breakdown(results_csv: Path):
    ...
    with results_csv.open(newline='', encoding='utf-8') as f:
        ...
        sym_col = next((reader.fieldnames[i] for i,k in enumerate(fields) if k in ("symbol","ticker")), reader.fieldnames[0])
        out_col = next((reader.fieldnames[i] for i,k in enumerate(fields) if k in ("outcome","result","status")), (reader.fieldnames+[""])[1])
        ...
        for row in reader:
            ...
            if not sym: continue
            counts[sym] += 1
            v = (row.get(out_col,"") or "").strip().upper()
            if v == "TP": wins[sym] += 1
...
def save_trades_breakdown(out_dir: Path, rows, totals, gap_map, preflight_band, news_map, news_path):
    ...
    header_cols = ["symbol","included_by","news_score","news_headline","gap_pct","in_band","traded","trades","tp","sl","pnl_sum"]
    ...
    for r in rows:
        ...
        lines.append(f"{sym},{included_by},{news_score},\"{head}\",{gap_str},{inband(gp)},{traded},{r['trades']},{r['tp']},{r['sl']},{r['pnl_sum']}")
    ...
    with csvp.open("w", newline="", encoding="utf-8") as f:
        ...
        for r in rows:
            ...
            gap_str     = "" if gp is None else f"{gp:.2f}"
            traded      = "yes" if r["trades"] > 0 else "no"
            w.writerow([sym, included_by, news_score, head, gap_str, inband(gp), traded, r["trades"], r["tp"], r["sl"], r["pnl_sum"]])
...
def main():
    ...
    # Run-only overrides via ENV (the CLI must support these)
    ...
    # 4) Env for helpers + CLI
    env = os.environ.copy(); env["PYTHONPATH"] = "src"

    # 5) Enrich gaps ALWAYS (for display and optional top-N)
    pairs, gap_map = enrich_gaps(pairs, args.date, env)

    # 6) Load catalyst news with min-score filter (tolerant headers)
    news_map, news_path = load_news_map(args.date, args.catalyst, args.news_min_score)

    # 7) Apply hybrid precedence and Top-N
    ...
    # 8) Catalyst news announce + counts
    ...
    # 9) Preflight gap band + messages
    min_gap, max_gap = read_scenario_gap_band(args.scenario)
    if min_gap is not None or max_gap is not None:
        print("[PREFLIGHT] Gap band from scenarios.json:",
    ...
    # 14) Per-symbol (with inclusion + news_score)
    rows, totals = per_symbol_breakdown(hybrid_csv)
    rows_by_sym = {r["symbol"]: r for r in rows}
    print("\n[PER-SYMBOL]")
    print("symbol   included_by  news_score  gap%    in_band  traded  trades  TP  SL    PnL   headline")
    for s in used_syms:
        gp = gap_map.get(s)
        gap_str = "" if gp is None else f"{gp:.2f}"
        in_band = ""
        ...
        print(f"{s:6}  {included_by:10}  {news_score:10}  {gap_str:>6}   {in_band:>6}   {traded:>6}   {r['trades']:6}  {r['tp']:2}  {r['sl']:2}  {r['pnl_sum']:>7.2f}  {head}")
