...
def main() -> None:
    ...
    # 1b) Decide whether to continue
    if UNIVERSE_PATH.exists():
        n = sum(1 for _ in UNIVERSE_PATH.open("r", encoding="utf-8", errors="ignore"))
        print(f"[UNIVERSE] {UNIVERSE_PATH} has {n} symbols")
        if n == 0:
            print(f"[INFO] {date_str} is likely a non-trading day (holiday/weekend) or no symbols met filters.")
            print("[INFO] Skipping minutes fetch and backtest.")
            return
    else:
...
# Stream summarize_results.py output to console and save to file
summary_path = out_dir / f"summary_{date_str}.txt"
with Popen([sys.executable, str(SCRIPTS / "summarize_results.py"), "--date", date_str], stdout=PIPE, stderr=PIPE, text=True, env=os.environ.copy()) as proc:
    buf = io.StringIO()
    ...
    summary_txt = buf.getvalue() if buf.getvalue() else "(no summary output)
"
