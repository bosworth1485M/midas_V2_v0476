...
#
# Notes:
# - Times are written in America/New_York (ET), DST-aware.
# - --session rth keeps only 09:30–16:00 ET.

import os, csv, json, urllib.request, argparse, pathlib
...
def fetch_minutes(symbol: str, day: str, key: str):
    ...
    url = f"https://api.polygon.io/v2/aggs/ticker/{symbol}/range/1/minute/{day}/{to}?adjusted=true&sort=asc&limit=50000&apiKey={key}"
        ...
        try:
            ...
            with out.open("w", newline="", encoding="ascii") as f:
                ...
                for x in rows:
                    ...
                    # Optional RTH filter
                    if args.session == "rth":
                        tt = ts_et.timetz()  # time with tzinfo
                        # Compare naive time component in ET
