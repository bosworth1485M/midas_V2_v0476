#!/usr/bin/env python3
# scripts/topgappers.py (prev-trading-day FIX ONLY + argparse compatibility)
# - Uses existing prev_trading_day_polygon.py to resolve prior TRADING day
# - Accepts/ignores extra args (e.g., --session) to match run_day_simple behavior
...
def main():
    ...
    ap.add_argument("--top",       type=int,   default=50)  # print only
    ap.add_argument("--out",       default=str(DEF_OUT))
    ap.add_argument("--no-write",  action="store_true")
    # Compatibility: accept session or other extras without error
    ap.add_argument("--session", default=None)  # ignored
    args, unknown = ap.parse_known_args()
    # ignore any other unknown tokens
