#!/usr/bin/env python3
# prepare_and_run_day_v4.py
# Simple daily runner:
# - Resolves previous TRADING day (for logging) via scripts/prev_trading_day_polygon.py
# - Parses topgappers table for the target date
# - Fetches minute bars for N tickers (0 = all)
# - Writes to ONE universe file only: data\universe_active.txt (overwrites each run)
# - Runs scenarios (default A,B,C,D,E)
# - Saves results under out\auto\<YYYYMMDD>\ <SCENARIO>\results_<YYYY-MM-DD>.csv
# - Prints summary
...
def main():
    ...
    # Write ONE universe file (overwrites)
