# scripts/run_range_and_summarize_top.py  (v1.5 = v1.4 + --top support)
# Fresh-run defaults:
...
def run_one_day(date_str: str, scenarios: list[str], args) -> bool:
    ...
    if args.top is not None:       cmd += ["--top", str(args.top)]  # NEW: forward Top-N trim
...
def main():
    ...
    # NEW: Top-N trim support (forwarded to run_day_simple.py)
    ap.add_argument("--top", type=int, default=None,
                    help="Trim universe to top N gappers per day (1..50). Forwarded to run_day_simple.py")
    ...
    # Validate --top (backwards compatible: None = unchanged behavior)
    if args.top is not None and not (1 <= args.top <= 50):
        ap.error("--top must be between 1 and 50")

    # Define a default output root internally
    out_root = Path(getattr(args, "out_root", "out/auto"))
    ...
    if args.save_csv:
        ...
        out_csv = out_root / f"range_summary_{tag}.csv"
    ...
    for d in daterange(start_dt, end_dt):
        ...
        if args.resume:
            all_exist = True
            for sc in scenarios:
                p = Path("out") / d.strftime("%Y%m%d") / sc / f"results_{date_str}.csv"
                if not p.exists():
        ...
        for sc in scenarios:
            p = Path('out') / d.strftime('%Y%m%d') / sc / f'results_{date_str}.csv'  # read from out/
