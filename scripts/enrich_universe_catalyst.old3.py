...
def enrich_and_pick(date_ymd: str,
    ...
    if not kept_symbols and allow_b_fill:
        ...
        if b_take:
            print(f"[FALLBACK] allow_b_fill=True → added {len(b_take)} B-grade symbols: {', '.join(b_take)}")
    ...
    if kept_symbols:
        ...
        print(f"[KEEP] {_s}  grade={_m.get('grade','?')} score={_m.get('score','?')}  headline={_trunc_headline(_m.get('headline') or 'NO HEADLINE')}")
    if len(kept_symbols) > 15:
        print(f"[KEEP] ...(and {len(kept_symbols)-15} more)")
    for _s in dropped[:15]:
        _m = meta_all.get(_s, {})
        print(f"[DROP] {_s}  grade={_m.get('grade','?')} score={_m.get('score','?')}  headline={_trunc_headline(_m.get('headline') or 'NO NEWS')}")
    if len(dropped) > 15:
