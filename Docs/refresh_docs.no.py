...
# Catalyst Hybrid sections, sync DEV_GUIDE scenario tables, optional PDF export,
# and auto-fix unclosed ``` code fences.
...
def sync_dev_guide_scenarios(root: Path, dev_guide_path: Path):
    ...
    log("[Docs] Synced scenarios and hybrid notes into DEV_GUIDE")
...
def main():
    parser = argparse.ArgumentParser(description="Refresh docs under Docs/. Inject inventory, run-day & hybrid sections, sync scenarios, optional PDF.")
    parser.add_argument("--root", default=".", help="Repo root (default: current directory).")
    parser.add_argument("--export", choices=["none", "pdf"], default="pdf", help="Export guides to PDF (default: pdf).")
    ...
    if ug_updated != ug_original:
        ...
        log(f"[Docs] Injected Run-Day & Hybrid sections into {ug_path}")
    else:
        log(f"[Docs] No user-guide section changes needed in {ug_path}")
    ...
    if dg_path.exists():
        ...
        log(f"[Docs] DEV_GUIDE not found at {dg_path}; skipping scenario sync")
