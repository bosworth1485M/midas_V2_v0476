...
# ``` code fences. Markdown only; no PDF export; single-write per file.
...
def generate_scripts_index(root: Path, out_path: Path) -> Tuple[str, List[str]]:
    ...
    md = "\n".join(lines)
    # Scripts index is regenerated each run — write unconditionally
...
def sync_dev_guide_scenarios_text(root: Path, dev_text: str) -> str:
    ...
    # Always ensure hybrid notes section exists/updated
...
def main():
    ...
    # 3) DEV_GUIDE: compute all changes and write once (if file exists)
